...
class ServicesView {
  ...
  render() {
    const serviceCards =
      Model.mockServices.length > 0
        ? Model.mockServices.map((s) => this.createServiceCard(s)).join("")
        : `<div class="col-span-full py-20 text-center"><p class="text-slate-500 font-black uppercase tracking-widest text-xs">No Services Defined</p></div>`;
    ...
    return `
      <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10 animate-fade-in">
        <div>
          ...
          <p class="text-slate-500 text-sm font-medium italic">Define standard labor rates and parts for your shop</p>
        </div>
        ...
        <div class="flex items-center gap-3 w-full md:w-auto">
          <div class="relative flex-1 md:w-72 group">
            ...
            <input type="text" placeholder="Search services..." oninput="window.handleServiceSearch(this.value)"
          ...
          <button onclick="window.openAddServiceModal()" 
           class="bg-emerald-600 hover:bg-emerald-500 text-slate-950 px-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all shadow-lg shadow-emerald-900/20 flex items-center gap-2 cursor-pointer active:scale-95">
            <span class="text-lg group-hover:rotate-90 transition-transform leading-none">+</span> <span>Add Service</span>
          </button>
        </div>
      </div>
  ...
  createServiceCard(service) {
    const labor = service.baseCost || 0;
    return `
      <div class="stat-card group hover:border-emerald-500/40 transition-all duration-300">
        <div class="flex justify-between items-start mb-6">
          <div>
            <h3 class="text-white font-bold text-lg tracking-tight group-hover:text-emerald-400 transition-colors">${service.name}</h3>
            <div class="flex items-center gap-2 mt-1">
              ...
              <span class="text-slate-500 text-[10px] uppercase font-black tracking-widest">${service.time} Standard Hours</span>
            </div>
          </div>
          <button onclick="window.handleDeleteService('${service.id}')" class="text-slate-700 hover:text-red-500 transition-colors cursor-pointer p-1.5 bg-slate-800/50 rounded-lg">
        ...
        <div class="grid grid-cols-2 gap-3 mb-6">
          ...
          <div class="bg-emerald-500/5 p-3 rounded-2xl border border-emerald-500/20 group-hover:bg-emerald-500/10 transition-all">
            ...
            <p class="text-sm font-black text-emerald-400">$${labor.toFixed(2)}</p>
          </div>
        </div>
      </div>`;
  }
  ...
  handleServiceSearch(query) {
    ...
    cards.forEach((card) => {
      ...
      const serviceName = (h3 ? h3.textContent : "").toLowerCase();
...
window.openAddServiceModal = () => servicesView.openAddServiceModal();
window.closeServiceModal = () => servicesView.closeServiceModal();
window.saveService = (event) => servicesView.saveService(event);
window.handleServiceSearch = (query) => servicesView.handleServiceSearch(query);
